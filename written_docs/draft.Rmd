...
plot.dat <- data.frame(group=NA, weighted_bias=NA, est=NA, 
                       lower=NA, upper=NA, bias=NA, metric=NA)
plot.dat2 <- data.frame(weighted_bias=NA, est=NA, 
                        lower=NA, upper=NA, bias=NA, metric=NA)
...
plot.dat2 %>% 
  ...
  ggplot(aes(x=weighted_bias, y=est)) +
    geom_point(data=raw_or, aes(x=value, y=odds_r, size=nschools), alpha=.05) +
    geom_line() +
