---
title             : "Race-based disparities in allocation of academic disciplinary actions are associated with county-level rates of bias"
...
We used measurements of implicit and explicit bias available from data collected through Project Implicit [@xu2014psychology]. From this data, we used only respondents who had geographic information that would allow us to place them in a United States county, identified as White, and visited the site before 2015. This consisted of approximately 1.1 million total respondents from `r dim(county_means)[1]` counties.
...
## Measures
The primary outcome in this analysis is a count of the number of students by race (black and white) who received one of several types of disciplinary action. We report here rates of out-of-school suspension, in-school suspension, school-related arrests, law enforcement referrals, and total number of expulsions of any type.
...
for (i in f){
  print(i)
  load(paste('/Users/travis/Documents/gits/educational_disparities/cluster/output/metrics_raw/inschool_susp/', i, sep=''))
  model <- m
  testing_model <- sample(f[f %ni% i], 1)
  load(paste('/Users/travis/Documents/gits/educational_disparities/cluster/output/metrics_raw/inschool_susp/', testing_model, sep=''))
  m$data$bias <- m$data$bias[,1]
  m$data$warmth <- m$data$warmth[,1]
  m$data$weighted_bias <- m$data$weighted_bias[,1]
  m$data$weighted_warmth <- m$data$weighted_warmth[,1]
  m$data$total_pop <- m$data$total_pop[,1]
  m$data$unemp_rate <- m$data$unemp_rate[,1]
  m$data$med_income <- m$data$med_income[,1]
  m$data$poverty_rate <- m$data$poverty_rate[,1]
  m$data$col_grads <- m$data$col_grads[,1]
  m$data$white_prop <- m$data$white_prop[,1]
  m$data$black_prop <- m$data$black_prop[,1]
  m$data$b.w.ratio <- m$data$b.w.ratio[,1]
  m$data %>%
    filter(!is.na(bias)) %>%
    filter(!is.na(warmth)) %>%
    filter(!is.na(weighted_bias)) %>%
    filter(!is.na(weighted_warmth)) %>%
    filter(!is.na(total_pop)) -> testing_data
...
test_summary_datweighted <- data.frame(file=f, 
                               r_2 = NA,
                               r_2_lower=NA,
                               r_2_upper=NA)
j=1
for (i in f){
  print(i)
  load(paste('/Users/travis/Documents/gits/educational_disparities/cluster/output/metrics_weighted/older_run/inschool_susp/', i, sep=''))
  model <- m
  testing_model <- sample(f[f %ni% i], 1)
  load(paste('/Users/travis/Documents/gits/educational_disparities/cluster/output/metrics_weighted/older_run/inschool_susp/', testing_model, sep=''))
  m$data$bias <- m$data$bias[,1]
  m$data$warmth <- m$data$warmth[,1]
  m$data$weighted_bias <- m$data$weighted_bias[,1]
  m$data$weighted_warmth <- m$data$weighted_warmth[,1]
  ...
  testing_data %>%
    ...
    summarise(r_2 = mean(r),
              r_2_lower = quantile(r, .25),
              r_2_upper = quantile(r, .75)) -> r2
  test_summary_datweighted[j,2:4] <- r2
  j = j+1
}
